...
class SI_Media_Audit extends SI_Abstract_Audit {
		...
		foreach ( $attachment_ids as $attachment_id ) {
			if ( 'attachment' !== get_post_type( $attachment_id ) ) {
				$issues[] = $this->issue(
					...
					sprintf(
						...
						__( 'Media reference (ID: %d) no longer exists.', 'site-inspector' ),
						$attachment_id
					),
			...
			if ( $file && ! file_exists( $file ) ) {
				$issues[] = $this->issue(
					...
					sprintf(
						/* translators: attachment ID */
						__( 'Media file (ID: %d) is missing from the server.', 'site-inspector' ),
						$attachment_id
					),
			...
			if ( $size > $threshold ) {
				...
				$issues[] = $this->issue(
					...
					sprintf(
						...
						__( 'Image "%1$s" is large (%2$d KB). Consider optimizing (recommended: under %3$d KB).', 'site-inspector' ),
						basename( $file ),
						$size_kb,
